...
def test_real_analysis_response_contains_metrics_without_network() -> None:
    ...
    assert response.metrics.candidate_core_files == 4
    ...
    assert response.context_quality_report.candidate_file_count == 4
    assert response.context_quality_report.selected_file_count == 2
    assert response.context_quality_report.omitted_candidate_count == 2
    ...
    assert response.context_quality_report.budget_limit_reached
    assert response.context_quality_report.selected_files == ["README.md", "package.json"]
    assert response.context_quality_report.directory_coverage[0].directory == "."
    assert response.context_quality_report.directory_coverage[0].selected_file_count == 2
    assert response.context_quality_report.omitted_candidates[0].path == "app/main.py"
    ...
    assert response.result_evaluation.coverage_score == 0.5
    ...
    assert metrics_payload["operation"] == "agent_analyze"
    assert metrics_payload["total_files"] == response.metrics.total_files
    assert metrics_payload["ignored_dirs"] == response.metrics.ignored_dirs
    assert metrics_payload["generated_doc_count"] == 1
    assert metrics_payload["selected_core_files"] == 2
    assert not metrics_payload["used_mock_ai"]
    assert metrics_payload["llm_call_count"] == 1
    assert metrics_payload["prompt_template_count"] == 7
    assert metrics_payload["generated_doc_count"] == 1
    assert metrics_payload["generated_doc_total_chars"] > 0
    assert metrics_payload["interview_question_count"] == 0
    assert metrics_payload["agent_step_count"] == response.metrics.agent_step_count
    assert metrics_payload["tool_call_count"] == response.metrics.tool_call_count
    assert metrics_payload["tool_success_count"] == response.metrics.tool_success_count
    assert metrics_payload["tool_failed_count"] == response.metrics.tool_failed_count
    assert metrics_payload["avg_tool_duration_ms"] == response.metrics.avg_tool_duration_ms
...
def test_job_workflow_emits_metrics_updated_with_scan_counts() -> None:
    ...
    assert repo_loaded_event.payload["context_quality_report"]["selected_file_count"] == 2
    assert repo_loaded_event.payload["context_quality_report"]["omitted_candidate_count"] == 1
...
def test_analysis_uses_llm_service_when_api_key_is_configured() -> None:
    ...
    assert metrics_payload["operation"] == "agent_analyze"
    assert not metrics_payload["used_mock_ai"]
    assert metrics_payload["provider"] == "deepseek"
    assert metrics_payload["model"] == "deepseek-v4-flash"
    assert metrics_payload["llm_call_count"] == 1
    assert metrics_payload["llm_success_count"] == 1
    assert metrics_payload["llm_total_duration_ms"] == 5
    assert metrics_payload["prompt_template_count"] == 7
    assert metrics_payload["generated_doc_count"] == 1
    assert metrics_payload["referenced_file_path_count"] == 1
    assert metrics_payload["interview_question_count"] == 0
