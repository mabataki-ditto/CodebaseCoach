...
from app.schemas.repo import BasicFileSummary, FileTreeNode, RepoRequest
...
class MetricsServiceTests(unittest.TestCase):
    def test_record_repo_operation_metrics_appends_jsonl(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            metrics_file = Path(tmp) / "data" / "metrics.jsonl"
        ...
        self.assertEqual(payload["operation"], "repo_parse")
        self.assertEqual(payload["status"], "success")
        self.assertEqual(payload["owner"], "owner")
        self.assertEqual(payload["duration_ms"], 120)
        self.assertIn("record_id", payload)

    def test_record_repo_operation_metrics_keeps_failure_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
        ...
        self.assertEqual(payload["error_code"], "REPO_CLONE_FAILED")
        self.assertEqual(payload["error_message"], "仓库克隆失败")

    def test_count_file_tree_nodes_counts_nested_nodes(self) -> None:
        tree = [
            FileTreeNode(
                name="src",
                path="src",
        ...
        self.assertEqual(count_file_tree_nodes(tree), 4)

    def test_build_mock_analysis_metrics_uses_real_character_counts(self) -> None:
        ...
        self.assertEqual(metrics.candidate_core_files, 4)
        self.assertEqual(metrics.selected_core_files, 2)
        self.assertEqual(metrics.read_files, 2)
        self.assertEqual(metrics.truncated_files, 1)
        self.assertEqual(metrics.raw_candidate_chars, 100)
        self.assertEqual(metrics.final_context_chars, 50)
        self.assertEqual(metrics.context_compression_ratio, 0.5)
        self.assertEqual(metrics.mock_doc_count, 2)
        self.assertEqual(metrics.mock_doc_total_chars, 5)
        self.assertEqual(metrics.analysis_duration_ms, 321)
        self.assertEqual(metrics.generated_doc_count, 2)
        self.assertEqual(metrics.generated_doc_total_chars, 5)
        self.assertEqual(metrics.generated_doc_total_words, 3)
        self.assertEqual(metrics.interview_question_count, 0)
        self.assertEqual(metrics.referenced_file_path_count, 0)
        self.assertEqual(metrics.llm_call_count, 0)
        self.assertEqual(metrics.llm_success_count, 0)
        self.assertEqual(metrics.llm_failed_count, 0)
        self.assertEqual(metrics.llm_total_duration_ms, 0)
        self.assertTrue(metrics.used_mock_ai)
        self.assertEqual(metrics.provider, "")
        self.assertEqual(metrics.model, "")
        self.assertEqual(metrics.prompt_template_count, 0)
    ...
    def test_build_mock_analysis_metrics_returns_zero_ratio_when_no_candidates(self) -> None:
        ...
        self.assertEqual(metrics.context_compression_ratio, 0)

    def test_parse_repo_records_success_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            metrics_file = Path(tmp) / "metrics.jsonl"
        ...
        self.assertEqual(payload["operation"], "repo_parse")
        self.assertEqual(payload["status"], "success")
        self.assertEqual(payload["repo_url"], "https://github.com/owner/repo")

    def test_scan_repo_records_success_metrics_without_network(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
        ...
        self.assertEqual(response.owner, "owner")
        self.assertEqual(payload["operation"], "repo_scan")
        self.assertEqual(payload["status"], "success")
        self.assertTrue(payload["cloned"])
        self.assertEqual(payload["file_tree_node_count"], 2)
        self.assertEqual(payload["basic_file_count"], 1)
        self.assertEqual(payload["basic_file_bytes"], 128)
